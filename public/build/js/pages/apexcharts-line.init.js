...
document.addEventListener("DOMContentLoaded", function () {
    console.log("Zoomable Timeseries Chart Loaded fahmid");
    var linechartZoomColors = getChartColorsArray("line_chart_zoomable");
    
    if (linechartZoomColors) {
        fetch('/user/usage-timeseries') // New endpoint you'll create
            .then(response => response.json())
            .then(data => {
                // Transform the data
                const seriesData = data.map(item => ({
                    x: new Date(item.date).getTime(), // Use the date string directly
                    y: item.value
                }));

                var options = {
                    series: [{
                        name: 'Your Metric', // Change this to your metric name
                        data: seriesData
                    }],
                    chart: {
                        type: 'area',
                    ...
                    title: {
                        text: 'Your Usage Over Time', // Change title
                        align: 'left',
                        style: {
                            fontWeight: 500,
                        },
                    ...
                    yaxis: {
                        ...
                        labels: {
                            ...
                            formatter: function(val) {
                                return val.toFixed(0); // Adjust formatting as needed
                            },
                        },
                        title: {
                            text: 'Usage', // Change y-axis title
                            style: {
                                fontWeight: 500,
                            },
                        },
                    ...
                    tooltip: {
                        ...
                        y: {
                            formatter: function(val) {
                                return val.toFixed(0); // Adjust tooltip formatting
                            }
                        }
                    }
                };

                var chartElement = document.querySelector("#line_chart_zoomable");
                if (!chartElement) {
